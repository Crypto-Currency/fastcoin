...
Fastcoin Core 1.8 introduces AuxPoW from block 371,337. AuxPoW is a technology
which enables miners to submit work done while mining other coins, as work
on the Fastcoin block chain. Fastcoin Core 1.8 also enables payment protocol
support for Fastcoin (note that Fastcoin's implementation of payment protocol is
...
as soon as possible afterwards in order to be able to continue to use Fastcoin.
Users remaining on the previous client **will not** be able to receive or spend
Fastcoin (with the exception of with other nodes who have not upgraded).
...
use "fastcoind" you must run it the first time with the "-reindex" command line
option.
...
AuxPoW benefits Fastcoin in two ways; firstly, the effective hashrate of the coin is
...
multipools from switching between Fastcoin and other Scrypt coins.
...
In comparison to the AuxPoW implementation in United Scrypt Coin, Fastcoin uses a
different coinbase transaction input script format which includes block height in
compliance with BIP0034.

## Payment Protocol

Fastcoin Core 1.8 adds payment protocol support, which is used to streamline payments
being made to merchants.

This protocol is defined in DIP0070-DIP0072, based on the BIP standards with the same
assigned numbers. DIP standards can be found at https://github.com/fastcoin/dips/ .
The key differencs between the Bitcoin and Fastcoin payment protocols are that
the Fastcoin payment request uses a "genesis" field containing the hash of the network's
...
(typically 1 FST, or around $0.00012 per transaction).
...
of Fastcoin compared to Bitcoin. This should lead to less false positives.

A problem in parsing mangled fastcoin: URIs under Windows has been resolved, and as
part of this work network detection for fastcoin: URIs is now more robust.
